<template>
  <div class="home">
    ...
    <HelloWorld v-for="(item,index) in items" :key="item.bin" :msg="item.name" :prodNo="item.id" :binNo="item.bin" :cardType="item.cardNetwork" :cardURL="getLogoURL(item.cardNetwork)" :bgColor="getLogoBgColor(item.cardNetwork)" :itemIndex="index" />
  </div>
</template>
