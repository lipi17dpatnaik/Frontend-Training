...
var numMovies = Object.keys(obj.shows).length; //gets number of movies
var i,j;
var movieIndex = 0;
var colCount = 3;
var rowCount = Math.ceil(numMovies/colCount);
var videosec = document.createElement('div');
videosec.className = 'videoSection';
document.body.appendChild(videosec);
//for (i=0; i<rowCount;i++){
    //var rowDiv = document.createElement('div');
    //rowDiv.className='row';
    //videosec.appendChild(rowDiv);

    for (movieIndex=0; movieIndex<numMovies; movieIndex++){
        //if (movieIndex === numMovies) {break;} // to not create extra divs
        var colDiv=document.createElement('div');
        colDiv.className='videoBoxes';
        videosec.appendChild(colDiv);
        var year = obj.shows[movieIndex].year;
        year = year.substring(0,year.length-3);

        //to get all move details
        colDiv.innerHTML = '<a href="#"><div class="content"><h3>' + 
        obj.shows[movieIndex].title + '</h3>' + 
        '<p>' + year + '</p>' +
